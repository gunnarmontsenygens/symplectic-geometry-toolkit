function [Phi_hist, PhiTPhi_hist, Psi_tilde_hist, Psi_tildeTPsi_tilde_hist, check_hist] = analyze_Phi_Psi_tilde_3dof(x0_vec, t_hist, params)
%==========================================================================
%
...
%==========================================================================
    ...
    for i = 1 : N_t
        ...
        Phi_hist.evals(i, :) = [D(1,1), D(2,2), D(3,3), D(4,4), D(5,5), D(6,6)];
        Phi_hist.norm_evals(i, :) = [norm(D(1,1)), norm(D(2,2)), norm(D(3,3)), norm(D(4,4)), norm(D(5,5)), norm(D(6,6))];
        Phi_hist.evecs(i, :, :) = V;
        ...
        PhiTPhi_hist.evals(i, :) = [D(1,1), D(2,2), D(3,3), D(4,4), D(5,5), D(6,6)];
        PhiTPhi_hist.norm_evals(i, :) = [norm(D(1,1)), norm(D(2,2)), norm(D(3,3)), norm(D(4,4)), norm(D(5,5)), norm(D(6,6))];
        ...
        Psi_tilde_mtx = compute_Psi_tilde_3dof(t0, tf, x0_vec, xf_vec, Phi_f_mtx, params);
        ...
        Psi_tilde_hist.evals(i, :) = [D(1,1), D(2,2), D(3,3), D(4,4), D(5,5), D(6,6)];
        Psi_tilde_hist.norm_evals(i, :) = [norm(D(1,1)), norm(D(2,2)), norm(D(3,3)), norm(D(4,4)), norm(D(5,5)), norm(D(6,6))];
        ...
        Psi_tildeTPsi_tilde_hist.evals(i, :) = [D(1,1), D(2,2), D(3,3), D(4,4), D(5,5), D(6,6)];
        Psi_tildeTPsi_tilde_hist.norm_evals(i, :) = [norm(D(1,1)), norm(D(2,2)), norm(D(3,3)), norm(D(4,4)), norm(D(5,5)), norm(D(6,6))];
        Psi_tildeTPsi_tilde_hist.evecs(i, :, :) = V;

        % CHECK PSI TILDE -------------------------------------------------
    
        % Compute check at t_hist(i)
        check = check_Psi_tilde_3dof(Psi_tilde_mtx, false);
        ...
        check_hist.detMm1(i) = check.detMm1;
